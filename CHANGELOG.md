...
- **Автоматический fallback механизм**: Groq → Perplexity → OpenAI
...
- Автоматическое определение провайдера по формату ключа (`gsk_`, `pplx-`, `sk-`)

### Исправлено (Fixed)

- **Shallow copy bug** — исправлена проблема с модификацией оригинальных параметров запроса
- **JSON инструкции** — теперь корректно добавляются для всех провайдеров (Groq, Perplexity)
